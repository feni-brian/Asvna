...
const FirstSection = () => {
	...
	const togglePlan = () => { setIsMonthlyPlan(!isMonthlyPlan); };
	...
	return (
		<div className="flex flex-col space-y-10  pb-20 items-center justify-center">
			<div className="space-y-6 flex flex-col text-center pt-20 px-10 md:px-0">
				...
				<div className="text-2xl text-gray-500">Access Birds features. No credit card required.</div>
			</div>
			<div className="flex px-10 md:px-0">
				<button className={`${isMonthlyPlan ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-600"} p-4 rounded-l-lg focus:outline-none w-40 md:w-80`} onClick={() => togglePlan()}>
					<div className="flex items-center justify-center space-x-4">
						<CalendarMinus size={24} />
						<div className="text-2xl">Monthly</div>
					</div>
				</button>
				<button className={`${isMonthlyPlan ? "bg-gray-100 text-gray-600" : "bg-blue-500 text-white"} p-4 rounded-r-lg focus:outline-none w-40 md:w-80`} onClick={() => togglePlan()}>
					<div className="flex items-center justify-center space-x-4">
			...
			<div>
				<div className={`${isMonthlyPlan ? "block" : "hidden"} text-center  text-2xl`} />
				<div className="md:flex p-10 md:space-x-10 space-y-10 md:space-y-0 items-center justify-center md:w-3/4 md:mx-auto">
					<div className="border-teal-500 p-10 rounded-lg border space-y-8 md:w-1/2">
						...
						<div>For teams that need to create project plans with confidence.</div>
						<div>$0</div>
						<div>Per user, per month billed annually $144.00</div>
						<div className="text-center">
							<button className="bg-blue-500 text-white px-4 py-2 rounded-lg text-xl w-full">Contact Sales</button>
						</div>
					...
					<div className="border-teal-500 p-10 rounded-lg border space-y-8 md:w-1/2">
						...
						<div className="text-xl">For teams that need to create project plans with confidence.</div>
						{isMonthlyPlan ? <div>$12.99</div> : <div>$129.99</div>}
						<div>Per user, per month billed annually $144.99</div>
						<div className="text-center">
							<button className="bg-blue-500 text-white px-4 py-2 rounded-lg text-xl w-full">Contact Sales</button>
						</div>
						<div className="text-2xl underline text-center">Purchase Now</div>
					...
					<div className="border-teal-500 p-10 rounded-lg border space-y-8 md:w-1/2">
						...
						<div className="text-xl">For teams that need to create project plans with confidence.</div>
						{isMonthlyPlan ? <div>$32.99</div> : <div>$329.99</div>}
						<div>Per user, per month billed annually $189.99</div>
						<div className="text-center">
							<button className="bg-blue-500 text-white px-4 py-2 rounded-lg text-xl w-full">Contact Sales</button>
						</div>
						<div className="text-2xl underline text-center">Purchase Now</div>
