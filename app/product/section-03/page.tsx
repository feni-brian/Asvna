...
const ThirdSection = () => {
	...
	const handleAccordianChange = (value: any) => {
			...
			<div className="flex space-x-10 px-10 items-center justify-center">
				<div>
					{options.map((option, index) => (
						<Accordion key={index} type="single" defaultValue="Marketing" onChange={handleAccordianChange}>
							<AccordionItem key={index} value={option.title}>
								<AccordionTrigger value={option.title} className="w-96 text-3xl">
									{option.title}
								</AccordionTrigger>
